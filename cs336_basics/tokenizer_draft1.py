...
def end_good(chunk: bytes, special_tokens: Optional[list[str]]) -> bool:
...
def pre_tokenize(text: bytes) -> list[bytes]:
    ...
    return [
        m.group()
        for m in re.finditer(r"""'(?i:[sdmt]|ll|ve|re)|[^\r\n\p{L}\p{N}]?+\p{L}+|\p{N}{1,3}| ?[^\s\p{L}\p{N}]++[\r\n]*|\s*[\r\n]|\s+(?!\S)|\s+""", _text)  # https://github.com/openai/tiktoken/pull/234
    ]


def process_chunk(chunk: bytes, special_tokens: Optional[list[str]], index: int, words_counter: dict[tuple[bytes, ...], int]):
    special_tokens = special_tokens or []
    for token in special_tokens:
        chunk = chunk.replace(token.encode("utf-8"), b"")
    for token in pre_tokenize(chunk):
        words_counter[
            tuple([i.encode() for i in token])
        ] += 1
    index += len(chunk)
    return index, words_counter


def get_words_counter(input_path: str, special_tokens: Optional[list[str]]) -> dict[list[bytes], int]:
    index = 0
    chunk_size = 4096
    ...
    while True:
        ...
        if end_good(chunk, special_tokens):
            index, words_counter = process_chunk(chunk, special_tokens, index, words_counter)
        else:
            forward_bytes = b""
            already_end = False
            while True:
                ...
                if end_good(chunk + forward_bytes, special_tokens):
                    index, words_counter = process_chunk(forward_bytes, special_tokens, index, words_counter)
                    break
            if already_end:
                break
...
def init_vocab(special_tokens: Optional[list[str]]) -> dict[int, bytes]:
    vocab = {
        i: bytes(i)
        for i in range(256)
    }
...
def get_best_pair(words_counter: dict[tuple[bytes, ...], int]) -> tuple[bytes, bytes] | None:
...
def merge(best_pair: tuple[bytes, bytes], vocab: dict[int, bytes], words_counter: dict[tuple[bytes, ...], int], merges: list[tuple[bytes, bytes]]):
